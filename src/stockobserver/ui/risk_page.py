...
def render() -> None:
    st.header("⚖️ Risk — Bridgewater lens")
    c1, c2 = st.columns([2, 1])
    symbol = c1.text_input("Symbol", "AAPL.US")
    benchmark = c2.text_input("Benchmark", "SPY.US")
    try:
        parse_symbol(symbol)
    ...
    if o.empty or b.empty:
        st.warning("No price data for the symbol or benchmark.")
        return
